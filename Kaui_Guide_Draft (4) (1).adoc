...
1. <<_basic_search>>
2. <<_advanced_search>>
3. <<part_5_plugin_manager_and_analytics,Plugin Manager and Analytics>>
4. <<part_4_tags_and_custom_fields,Tags and Custom Fields>>
5. <<part_3_users_roles_permissions,Users,>> <<part_6_tenants_and_admin,Tenants, and Admin>>
...
*Note:* This icons does not display if the only payment method listed is EXTERNAL_PAYMENT.
...
image::ShowTopSearch.png[width=450]
...
[NOTE]
*Note:* In addition to searching with an ID, some object types can be searched for using an external key. The object types that include an external key include: account, bundle, payment, subscription, transaction.
...
|Bill cycle day
|The day of the month on which the system generates an invoice for this account. This field applies to accounts that are subscribed to monthly subscriptions (or a multiple of monthly, such as quarterly, annually, etc.).

By default, the system sets this day to be the day the customer signed up for their first subscription. However, the bill cycle day (BCD) for can vary depending on the billing alignment rules specified in the catalog.

For more information, see https://docs.killbill.io/latest/userguide_subscription.html#_billing_alignment_rules[Billing Alignment Rules] in the _Subscription_ manual.
...
|Plugin name
|Type in the name of the plugin that is associated with this type of payment method. [ASK RESHMA]
